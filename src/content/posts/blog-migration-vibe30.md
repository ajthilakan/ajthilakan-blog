...
**Agent pipeline:** I also set up subagents on my NanoClaw instance — `blog-drafter`, `blog-reviewer`, and `blog-publisher` — that work off my Obsidian notes to generate an initial draft based on my outline, run a review pass, and open a draft PR on my blog repo. More on that below.

**Branch structure:** `main` and `drafts` branches, with branch protection on `main`. The publishing agent can only open PRs targeting `drafts` — it can never merge to `main`. I review the PR, check the Cloudflare preview, and do the final merge myself.

![Screenshot showing the branch and PR workflow — image to be added to repo](/assets/blog-migration-vibe30/workflow-branches.png)

**Preview deployments:** Cloudflare Pages builds a unique preview URL for every PR branch. Before anything goes live, I can see exactly what it'll look like at a real URL. This was a pleasant surprise — I hadn't set it out to work this way, but it ended up being one of the more useful parts of the workflow.

![Screenshot of a Cloudflare Pages preview deployment URL for a pull request — image to be added to repo](/assets/blog-migration-vibe30/cloudflare-preview.png)

**DNS cutover:** Moved my domain from WordPress DNS over to Cloudflare Pages. Non-destructive until the final flip.
...
![Illustration about AI-assisted work and the risk of slop (Credit: Liz Fosslien) — image to be added to repo](/assets/blog-migration-vibe30/fosslien-ai-work.png)
*Credit: Liz Fosslien*
